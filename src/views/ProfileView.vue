...
import { useToastStore } from '../stores/toast' // Подключили стор Тостов

const toast = useToastStore() // Инициализировали тост
...
const isDeleting = ref(false) // Защита от двойного клика при удалении

const showEditModal = ref(false)
const isSaving = ref(false) // Защита от двойного клика при сохранении
...
const handleEditFileChange = (event: Event) => {
  ...
  if (target.files) {
    selectedEditFiles.value = Array.from(target.files)
  }
}
...
const submitEdit = async () => {
  isSaving.value = true // Блокируем кнопку сохранения
  try {
    ...
    selectedEditFiles.value.forEach(file => {
    ...
    if (index !== -1) {
      myAds.value[index] = response.data
    }
    closeEditModal()
    ...
    isSaving.value = false // Разблокируем
  }
}
  ...
  <div class="max-w-4xl mx-auto mt-10 px-4 text-white pb-20">
    ...
    <div v-if="user" class="bg-neutral-800 p-8 rounded-2xl border border-neutral-700 shadow-xl mb-8">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <p class="text-2xl font-bold mb-1 break-all">{{ user.email }}</p>
        ...
        <button @click="toggleRole" class="bg-neutral-700 hover:bg-neutral-600 border border-neutral-500 px-6 py-3 rounded-xl font-bold transition-colors">
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
      <div class="bg-neutral-800 p-6 rounded-2xl border border-neutral-700 w-full max-w-sm shadow-2xl">
        ...
        <div class="flex justify-end gap-3">
          ...
          <!-- АНТИ-ДУБЛЬ: кнопка удаления меняет текст и блокируется -->
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
      <div class="bg-neutral-800 p-6 rounded-2xl border border-neutral-700 w-full max-w-2xl shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <form @submit.prevent="submitEdit" class="flex flex-col gap-4">
          <div>
            <label class="block text-gray-400 text-sm mb-1">Название</label>
            <input v-model="editForm.title" type="text" class="w-full bg-neutral-900 border border-neutral-700 text-white rounded-xl p-3 outline-none focus:border-blue-500" required>
          </div>
          
          <div class="flex gap-4">
            <div class="w-1/3">
              <label class="block text-gray-400 text-sm mb-1">Цена</label>
              <div class="flex">
                <input v-model="editForm.price" type="number" class="w-full bg-neutral-900 border border-neutral-700 text-white rounded-l-xl p-3 outline-none focus:border-blue-500" required>
                <select v-model="editForm.currency" class="bg-neutral-800 border border-neutral-700 border-l-0 text-white rounded-r-xl px-2 outline-none">
            ...
            <div class="w-1/3">
              <label class="block text-gray-400 text-sm mb-1">Город</label>
              <input v-model="editForm.city" type="text" class="w-full bg-neutral-900 border border-neutral-700 text-white rounded-xl p-3 outline-none focus:border-blue-500" required>
            </div>
            <div class="w-1/3">
              <label class="block text-gray-400 text-sm mb-1">Телефон</label>
              <input v-model="editForm.phone_number" type="tel" class="w-full bg-neutral-900 border border-neutral-700 text-white rounded-xl p-3 outline-none focus:border-blue-500" required>
            </div>
          </div>

          <div v-if="editForm.images && editForm.images.length > 0">
            <label class="block text-gray-400 text-sm mb-2">Текущие фото (нажми X, чтобы удалить)</label>
            <div class="flex gap-3 overflow-x-auto pb-2">
              <div v-for="img in editForm.images" :key="img.id" class="relative w-24 h-24 flex-shrink-0 bg-neutral-900 rounded-xl border border-neutral-700">
                <img :src="getImageUrl(img.image)" class="w-full h-full object-cover rounded-xl" />
                <button type="button" @click="removeExistingImage(img.id)" class="absolute -top-2 -right-2 bg-red-600 hover:bg-red-700 text-white rounded-full w-6 h-6 flex items-center justify-center font-bold text-xs shadow-lg">
                  ✕
                </button>
              </div>
            </div>
          ...
          <div>
            <label class="block text-gray-400 text-sm mb-1">Добавить новые фото</label>
            <input 
              type="file" 
              multiple 
              accept="image/*"
              @change="handleEditFileChange"
              class="w-full bg-neutral-900 border border-neutral-700 text-white rounded-xl p-2 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-neutral-700 file:text-white hover:file:bg-neutral-600 outline-none cursor-pointer"
            >
          </div>

          <div>
            <label class="block text-gray-400 text-sm mb-1">Описание</label>
            <textarea v-model="editForm.description" rows="3" class="w-full bg-neutral-900 border border-neutral-700 text-white rounded-xl p-3 outline-none focus:border-blue-500"></textarea>
          </div>
          ...
          <div class="flex justify-end gap-3 mt-4">
            ...
            <!-- АНТИ-ДУБЛЬ: кнопка сохранения меняет текст и блокируется -->
            <button type="submit" :disabled="isSaving" class="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-bold transition-colors">
